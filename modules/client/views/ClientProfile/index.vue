...
const apiBaseUrl = `/v1/client/profiles`
...
const handleView = async (row) => {
  const id = row.id
  modalStore.toggleModalUsage(false) // if you want to navigate to route set to false
  metaStore.setMeta(
    'clientExtras',
    {
      currencyId: row.ClientBillingInformation?.[0]?.currency,
      countryId: row.ClientBillingInformation?.[0]?.tax_jurisdiction,
    },
    { flash: false } 
  )
...
//const handleEdit = (id) => {
//   router.push({ name: 'client/clientprofile/update', params: { id } });
//}
...
const handleEdit = async (row) => {
  ...
  const handleSubmit = async (updatedData) => {
    ...
    //   title: 'Success',
    //   icon: 'success',
    //   text: data.value?.alertifyPayload?.message} ?? 'ClientProfile Updated successfully',
...
const handleCreate = async () => {
  ...
  if (!modalStore.useModal) {
    router.push({ name: 'client/clientprofile/create' })
    return
  }

  // Define form submission handler
  const handleSubmit = async (newData) => {
    const apiBaseUrl = `/v1/client/profile`
    ...
    //   title: 'Success',
    //   icon: 'success',
    //   text: data.value?.alertifyPayload?.message} ?? 'ClientProfile Created successfully',
    //   showConfirmButton: false,
    //   timer: 2000,
    //   timerProgressBar: true,
    // })
...
const handleDelete = async (row) => {
  ...
  if (result.isConfirmed) {
    try {
      ...
      const { data, request, isLoading } = useApi(`/v1/client/client-profile/${id}`, {
        method: 'DELETE',
      })
...
<template>
  <div class="card p-3">
    <div class="row d-flex justify-content-between align-items-center mb-3">
      <div class="col-auto">
        <h1 class="h4 mt-2">List of ClientProfile</h1>
      </div>
