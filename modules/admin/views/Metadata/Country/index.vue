...
const apiBaseUrl = `/v1/admin/countries`
...
const handleView = async (row) => {
  ...
  if (!modalStore.useModal) {
    router.push({ name: 'iam/country/view', params: { id } })
    return
  }
...
const handleEdit = async (row) => {
  ...
  const handleSubmit = async (updatedData) => {
    ...
    //   title: 'Success',
    //   icon: 'success',
    //   text: `${data.value?.alertifyPayload?.message}` ?? 'Country Updated successfully',
...
const handleCreate = async () => {
  ...
  if (!modalStore.useModal) {
    router.push({ name: 'iam/country/create' })
    return
  }

  // Define form submission handler
  const handleSubmit = async (newData) => {
    const apiBaseUrl = `/v1/admin/country`
    ...
    proxy.$showAlert({
      title: 'Success',
      icon: 'success',
      text: `${data.value?.alertifyPayload?.message}` ?? 'Country Created successfully',
      showConfirmButton: false,
      timer: 2000,
      timerProgressBar: true,
    })
...
<template>
  <div class="card p-3">
    <div class="row d-flex justify-content-between align-items-center mb-3">
      <div class="col-auto">
        <h1 class="h4 mt-2">List of Countries</h1>
      </div>
      <div class="col-auto mb-4">
        <Button
          ...
          @click="handleCreate"
          v-tooltip:left="'Add New Country'"
        >
          New Country
        </Button>
      </div>
