...
const apiBaseUrl = `/v1/admin/currencies`
...
const handleView = async (row) => {
  ...
  if (!modalStore.useModal) {
    router.push({ name: 'iam/currency/view', params: { id } })
    return
  }

  const apiBaseUrl = `/v1/admin/currency/name?id=${id}`
...
const handleEdit = async (row) => {
  ...
  if (!modalStore.useModal) {
    ...
    router.push({ name: 'iam/currency/update', params: { id } })
    return
  }

  // Fetch appointment data before opening the modal
  const apiBaseUrl = `/v1/admin/currency/name?id=${id}`
  const { data, request, isLoading } = useApi(apiBaseUrl,{
    method: 'GET',
    options: {},
    autoFetch: true,
  })
  ...
  const handleSubmit = async (updatedData) => {
    const { data, request: updateData, error } = useApi(`/v1/admin/currency/${id}`, {method: 'PUT'})
    ...
    proxy.$showAlert({
      title: 'Success',
      icon: 'success',
      text: `${data.value?.alertifyPayload?.message}` ?? 'Currency Updated successfully',
...
const handleCreate = async () => {
  ...
  if (!modalStore.useModal) {
    router.push({ name: 'iam/currency/create' })
    return
  }
  ...
  const handleSubmit = async (newData) => {
    ...
    proxy.$showAlert({
      title: 'Success',
      icon: 'success',
      text: `${data.value?.alertifyPayload?.message}` ?? 'Currency Created successfully',
      showConfirmButton: false,
      timer: 2000,
      timerProgressBar: true,
    })
...
const handleDelete = async (row) => {
  ...
  if (result.isConfirmed) {
    try {
      ...
      const { data, request, isLoading } = useApi(`/v1/admin/currency/${id}`, {method: 'DELETE'})
...
<template>
  <div class="card p-3">
    <div class="row d-flex justify-content-between align-items-center mb-3">
      <div class="col-auto">
        <h1 class="h4 mt-2">List of Currencies</h1>
      </div>
      <div class="col-auto mb-4">
        <Button
          ...
          @click="handleCreate"
          v-tooltip:left="'Add New Currency'"
        >
          New Currency
        </Button>
      </div>
      ...
      >
        ...
        <template #column-status="{ row }">
          <span :class="getStatusClass(row.status?.theme)">
            {{ row.status.label }}
          </span>
        </template>
      </OmniGridView>
