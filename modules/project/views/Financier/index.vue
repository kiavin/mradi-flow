...
const apiBaseUrl = `/v1/project/financiers`
...
// router.push({ name: 'project/financier/view', params: { id } });
//};
...
const handleView = async (row) => {
  ...
  const apiBaseUrl = `/v1/project/financier/${id}`
...
//const handleEdit = (id) => {
//   router.push({ name: 'project/financier/update', params: { id } });
//}
...
const handleEdit = async (row) => {
  ...
  const handleSubmit = async (updatedData) => {
    ...
    //   title: 'Success',
    //   icon: 'success',
    //   text: data.value?.alertifyPayload?.message} ?? 'Financier Updated successfully',
...
const handleCreate = async () => {
  ...
  if (!modalStore.useModal) {
    router.push({ name: 'project/financier/create' })
    return
  }

  // Define form submission handler
  const handleSubmit = async (newData) => {
    const apiBaseUrl = `/v1/project/financier`
    ...
    //   title: 'Success',
    //   icon: 'success',
    //   text: data.value?.alertifyPayload?.message} ?? 'Financier Created successfully',
    //   showConfirmButton: false,
    //   timer: 2000,
    //   timerProgressBar: true,
    // })
...
const handleDelete = async (row) => {
  ...
  if (result.isConfirmed) {
    try {
      ...
      const { data, request, isLoading } = useApi(`/v1/project/financier/${id}`, {
        method: 'DELETE',
      })
