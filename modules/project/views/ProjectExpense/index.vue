...
const handleView = async (row) => {
  ...
  const apiBaseUrl = `/v1/project/project-expense/${id}`
...
//const handleEdit = (id) => {
//   router.push({ name: 'project/projectexpense/update', params: { id } });
//}
...
const handleEdit = async (row) => {
  ...
  const handleSubmit = async (updatedData) => {
    ...
    //   title: 'Success',
    //   icon: 'success',
    //   text: data.value?.alertifyPayload?.message} ?? 'ProjectExpense Updated successfully',
...
const handleDelete = async (row) => {
  ...
  if (result.isConfirmed) {
    try {
      ...
      const { data, request, isLoading } = useApi(`/v1/project/project-expense/${id}`, {
        method: 'DELETE',
      })
...
<template>
  <div class="card p-3">
    <div class="row d-flex justify-content-between align-items-center mb-3">
      <div class="col-auto">
        <h1 class="h4 mt-2">List of ProjectExpense</h1>
      </div>
