...
const apiBaseUrl = `/v1/project/project-financiers`
...
// router.push({ name: 'project/projectfinancier/view', params: { id } });
//};
...
const handleView = async (row) => {
  ...
  modalStore.openModal(
    ...
    'View ProjectFinancier',
  )
}

//const handleEdit = (id) => {
//   router.push({ name: 'project/projectfinancier/update', params: { id } });
//}
...
const handleCreate = async () => {
  ...
  const handleSubmit = async (newData) => {
    ...
    //   title: 'Success',
    //   icon: 'success',
    //   text: data.value?.alertifyPayload?.message} ?? 'ProjectFinancier Created successfully',
    //   showConfirmButton: false,
    //   timer: 2000,
    //   timerProgressBar: true,
    // })
...
const handleDelete = async (row) => {
  ...
  if (result.isConfirmed) {
    try {
      ...
      const { data, request, isLoading } = useApi(`/v1/project/project-financier/${id}`, {
        method: 'DELETE',
      })
